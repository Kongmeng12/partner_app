...
/// Prices and open/closed status by date range, per room type.
///
...
///
/// Reached from the Calendar's pricing button; who is in which room lives on
/// the Calendar itself.
...
class _PricingScreenState extends ConsumerState<PricingScreen> {
  ...
  void _toggle(CalendarDay day) {
    if (_rangeAnchor != null) {
      setState(() {
        _selected
          ..clear()
          ..addAll(isosBetween(_rangeAnchor!, day.date));
      });
      return;
    }
    setState(() {
      _selected.contains(day.date)
      ...
      _selected
        ...
        ..addAll(
          days
              .where((d) {
                final weekday =
                    DateTime.parse('${d.date}T00:00:00.000Z').weekday;
  ...
  Future<void> _applyToSelection({
    ...
    final chosen =
        days.where((d) => _selected.contains(d.date)).toList()
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ລາຄາ & ເປີດ-ປິດຂາຍ')),
      body: roomTypes.when(
        ...
        data: (list) {
          ...
          final calendar = ref.watch(
            roomCalendarProvider((roomTypeId: roomTypeId, month: month)),
          );
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Row(
                            children: [
                              QuickChip(
                                label: 'ທັງເດືອນ',
                                onTap: () => _selectAll(days),
                              ),
                              const SizedBox(width: 8),
                              QuickChip(
                                label: 'ເສົາ-ອາທິດ',
                                onTap: () => _selectWeekends(days),
                              ),
                            ],
                          ),
                        ),
  ...
  String _monthLabel(DateTime month) =>
      '${_laoMonths[month.month - 1]} ${month.year}';
}
              ...
              children: [
                ...
                if (day != null)
                  Text(
                    closed
                        ? 'ປິດຂາຍ'
                        : kipShort(day!.price).replaceFirst('₭', ''),
                    maxLines: 1,
